  ...
  static function on_page_parts_dynamic_build($event, $id = null) {
    if ($id === null) {
      foreach (entity::get('poll')->instances_select() as $c_poll) {
        page_part_preset::insert('poll_form_'.$c_poll->id, translation::get('Poll').' (SQL)', $c_poll->question, ['content' => 'content'], null, 'link', 'forms/polls/poll', ['_id_poll' => $c_poll->id], [], 0, 'polls');
      }
    }
