...
namespace effcore {
  ...
  static function         date_format($date)     {$date = \DateTime::createFromFormat('Y-m-d',       $date,     new \DateTimeZone('UTC') );                                   if ($date) return $date->setTime    (0, 0)                                            ->format(module::settings_get('locales')->format_date    );}
  static function         time_format($time)     {$date = \DateTime::createFromFormat(      'H:i:s', $time,     new \DateTimeZone('UTC') );                                   if ($date) return $date->setTimezone( new \DateTimeZone(core::timezone_client_get()) )->format(module::settings_get('locales')->format_time    );}
  static function     datetime_format($datetime) {$date = \DateTime::createFromFormat('Y-m-d H:i:s', $datetime, new \DateTimeZone('UTC') );                                   if ($date) return $date->setTimezone( new \DateTimeZone(core::timezone_client_get()) )->format(module::settings_get('locales')->format_datetime);}
  static function     timestmp_format($timestmp) {$date = \DateTime::createFromFormat('U',           $timestmp + core::timezone_offset_sec_get(core::timezone_client_get())); if ($date) return $date                                                               ->format(module::settings_get('locales')->format_datetime);}
