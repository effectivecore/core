  ...
  static function block_menu_languages($page) {
    ...
    foreach ($languages as $c_language) {
      ...
      if (url::is_active($c_href))
           $c_link = new markup('a', ['href' => $c_href, 'title' => translation::apply('go to %%_language language', ['language' => $c_language->title_en], 'en'), 'aria-current' => 'true'], new text_simple($c_title));
      else $c_link = new markup('a', ['href' => $c_href, 'title' => translation::apply('go to %%_language language', ['language' => $c_language->title_en], 'en')                          ], new text_simple($c_title));
