...
namespace effcore\modules\storage {
  ...
  static function on_block_presets_dynamic_build($event, $id = null) {
    if ($id === null                                        ) {foreach (selection::get_all('sql') as $c_item)        block_preset::insert('selection_sql__'.$c_item->id, 'Selections', $c_item->title ?: 'NO TITLE', [ /* no areas */ ], /* display = */ null, 'code', '\\effcore\\modules\\storage\\events_page::block_markup__selection_get', [ /* no properties */ ], ['id' => $c_item->id], 0, 'storage');}
    if ($id !== null && strpos($id, 'selection_sql__') === 0) {$c_item__id = substr($id, strlen('selection_sql__')); block_preset::insert('selection_sql__'.$c_item__id, 'Selections',                   'NO TITLE', [ /* no areas */ ], /* display = */ null, 'code', '\\effcore\\modules\\storage\\events_page::block_markup__selection_get', [ /* no properties */ ], ['id' => $c_item__id], 0, 'storage');}
  }
