...
namespace effcore\modules\storage {
  ...
  static function on_block_presets_dynamic_build($event, $id = null) {
    if ($id === null                                               ) {foreach (selection::get_all('sql') as $c_item)               block_preset::insert('block__selection_sql__'.$c_item->id, 'Selections', $c_item->title ?: 'NO TITLE', [ /* all areas */ ], ['type' => 'code', 'source' => '\\effcore\\modules\\storage\\events_page::block_markup__selection_get', 'args' => ['instance_id' => $c_item->id, 'entity_name' => 'selection'], 'has_admin_menu' => true, 'attributes' => ['data-section' => true, 'data-id' => 'block__selection_sql__'.$c_item->id]], 0, 'storage');}
    if ($id !== null && strpos($id, 'block__selection_sql__') === 0) {$c_item__id = substr($id, strlen('block__selection_sql__')); block_preset::insert('block__selection_sql__'.$c_item__id, 'Selections',                   'NO TITLE', [ /* all areas */ ], ['type' => 'code', 'source' => '\\effcore\\modules\\storage\\events_page::block_markup__selection_get', 'args' => ['instance_id' => $c_item__id, 'entity_name' => 'selection'], 'has_admin_menu' => true, 'attributes' => ['data-section' => true, 'data-id' => 'block__selection_sql__'.$c_item__id]], 0, 'storage');}
  }
