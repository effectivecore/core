      ...
      case 'update':
        if (!empty($form->_instance)) {
          ...
          if ($form->_instance->update())
               message::insert(new text('Item of type "%%_name" with id = "%%_id" was updated.',     ['name' => translation::get($entity->title), 'id' => implode('+', $entity->id_get_from_values($form->_instance->values_get())) ])           );
          else message::insert(new text('Item of type "%%_name" with id = "%%_id" was not updated!', ['name' => translation::get($entity->title), 'id' => implode('+', $entity->id_get_from_values($form->_instance->values_get())) ]), 'warning');
        }
