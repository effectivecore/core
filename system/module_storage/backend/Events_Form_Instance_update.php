        ...
        if ($form->has_error_on_build === false &&
            ...
            foreach ($entity->fields as $c_name => $c_field) {
                if (!empty($c_field->managing_is_enabled_on_update) &&
                    ...
                    if     (empty($c_field->managing_control_value_manual_set) && is_object($c_control) && $c_control instanceof control && $c_control instanceof field_checkbox !== true) $c_control->  value_set($form->_instance->{$c_name}, ['once' => true]);
                    elseif (empty($c_field->managing_control_value_manual_set) && is_object($c_control) && $c_control instanceof control && $c_control instanceof field_checkbox === true) $c_control->checked_set($form->_instance->{$c_name});
                }
            }
