          ...
          if (!empty($form->_selected_instances)) {
            foreach ($form->_selected_instances as $c_instance_id => $c_instance) {
              if ($items['#actions']->value_get() === 'delete') {
                ...
                } else                                                     message::insert(new text('Item of type "%%_type" with ID = "%%_id" was not deleted because it\'s embed!', ['type' => translation::apply($entity->title), 'id' => $c_instance_id]), 'warning');
              }
            }
          }
