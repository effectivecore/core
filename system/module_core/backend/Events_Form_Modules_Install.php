  ...
  static function on_init($event, $form, $items) {
    ...
    foreach ($modules_by_groups as $c_modules) {
      foreach ($c_modules as $c_module) {
        ...
        foreach ($c_dependencies->php as $c_id => $c_state) $c_dependencies_php_items->child_insert(new markup('x-sticker', ['data-style' => $c_state ? null : 'warning'], [new markup('x-title', [], new text_simple(strtoupper($c_id))), new markup('x-version', [],                        $c_module->dependencies->php   [$c_id] )] ), strtolower($c_id));
        foreach ($c_dependencies->sys as $c_id => $c_state) $c_dependencies_sys_items->child_insert(new markup('x-sticker', ['data-style' => $c_state ? null : 'warning'], [new markup('x-title', [], new text_simple(strtoupper($c_id))), new markup('x-version', [], locale::format_version($c_module->dependencies->system[$c_id]))] ), strtolower($c_id));
        foreach ($c_depended          as $c_id => $c_state) $c_depended_sys_items    ->child_insert(new markup('x-sticker', ['data-style' => $c_state ? 'ok' :   null   ], [new markup('x-title', [], new text_simple(strtoupper($c_id)))]                                                                                              ), strtolower($c_id));
        ...
        $c_switcher->disabled_set(isset($embedded[$c_module->id]) || !$c_is_ok_php_dependencies || !$c_is_ok_sys_dependencies || !$c_is_ok_sys_depended);
            ...
            foreach ($c_dependencies as $c_id => $c_version_min) {
              if (isset($modules_to_disable[$c_id])) {
                ...
                $items['#is_enabled:'.$c_module->id]->error_set(
                  'Module "%%_title" (%%_id) cannot be enabled when you try to disable dependent module "%%_dependent_title" (%%_dependent_id)!', [
                  'title'           => $c_module->title,
                  'id'              => $c_module->id,
                  'dependent_title' => module::get($c_id)->title,
                  'dependent_id'    => module::get($c_id)->id
                ]);
              }
            }
          }
