  ...
  static function on_init($event, $form, $items) {
    ...
    foreach ($modules_by_groups as $c_modules) {
      foreach ($c_modules as $c_module) {
        ...
        foreach ($c_dependencies->php as $c_id => $c_state) $c_dependencies_php_items->child_insert(new markup('x-sticker', ['data-state' => $c_state ? null : 'warning'], [new markup('x-title', [], new text_simple(strtoupper($c_id))), new markup('x-version', [],                        $c_module->dependencies->php   [$c_id] )] ), strtolower($c_id));
        foreach ($c_dependencies->sys as $c_id => $c_state) $c_dependencies_sys_items->child_insert(new markup('x-sticker', ['data-state' => $c_state ? null : 'warning'], [new markup('x-title', [], new text_simple(strtoupper($c_id))), new markup('x-version', [], locale::format_version($c_module->dependencies->system[$c_id]))] ), strtolower($c_id));
        foreach ($c_depended          as $c_id => $c_state) $c_depended_sys_items    ->child_insert(new markup('x-sticker', ['data-state' => $c_state ? 'ok' :   null   ], [new markup('x-title', [], new text_simple(strtoupper($c_id)))]                                                                                              ), strtolower($c_id));
