  ...
  static function on_init($event, $form, $items) {
    ...
    foreach ($modules_by_groups as $c_modules) {
      foreach ($c_modules as $c_module) {
        ...
        if ($c_module->icon_path                              ) $c_info->child_insert(new markup('x-param', ['data-type' => 'icon'              ], [new markup('x-title', ['aria-hidden' => 'true'], 'icon'                      ), new markup('x-value', [], new markup_simple('img', ['src' => $c_module->icon_path[0] == '/' ? $c_module->icon_path : '/'.$c_module->path.$c_module->icon_path]) )]), 'icon'              );
            ...
            foreach ($c_dependencies as $c_id => $c_version_min) {
              if (isset($modules_to_disable[$c_id])) {
                ...
                $items['#is_enabled:'.$c_module->id]->error_set(
                  'Can not enable module "%%_module_id" when you try to disable dependent module "%%_dependent_module_id"!', [
                  'module_id'           => $c_module->id,
                  'dependent_module_id' => $c_id
                ]);
              }
            }
          }
