    ...
    if (!isset($this->display) ||
      ...
      switch ($this->type) {
        ...
        case 'link': if ($this->type === 'copy') $result = core::deep_clone(storage::get('files')->select($this->source, true));
                     ...
                     $this->child_insert($result);                                                                       return;
        case 'code': $this->child_insert(call_user_func_array($this->source, ['page' => $page, 'args' => $this->args])); return;
        case 'text': $this->child_insert(new text($this->source));                                                       return;
        default    : $this->child_insert(         $this->source );                                                       return;
      }
    }
  }
