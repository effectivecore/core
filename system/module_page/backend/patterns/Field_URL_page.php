...
class Field_URL_page extends Field_URL {
    ...
    function render_description() {
        ...
        $this->description['url-page-api'    ] = new Markup('p', ['data-id' => 'url-page-api'    ], new Text('Field value cannot be start with "%%_value".', ['value' => '/api/'    ]));
    ...
    static function validate_value($field, $form, $element, &$new_value) {
        if (parent::validate_value($field, $form, $element,  $new_value) === true) {
            ...
            if (strlen($new_value) && preg_match('%^/api(/.*|)$%',     $new_value)) {$field->error_set(new Text('Value of "%%_title" field cannot be start with "%%_value"!', ['title' => (new Text($field->title))->render(), 'value' => '/api/'    ])); return;}
            return true;
        }
    }
