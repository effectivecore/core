  ...
  # validate each item
    ...
    foreach ($new_values as $c_new_value) {
      ...
      switch ($c_new_value->error) {
        case UPLOAD_ERR_INI_SIZE  : $field->error_set(translation::get('Field "%%_title" after trying to upload the file returned an error: %%_error!', ['title' => translation::get($field->title), 'error' => translation::get('the size of uploaded file more than %%_size', ['size' => locale::human_bytes_format($max_size)])])); return;
      ...
      if ($c_new_value->size > $max_size)        {$field->error_set(translation::get('Field "%%_title" after trying to upload the file returned an error: %%_error!', ['title' => translation::get($field->title), 'error' => translation::get('the size of uploaded file more than %%_size', ['size' => locale::human_bytes_format($max_size)])])); return;}
    }
    return true;
  }
