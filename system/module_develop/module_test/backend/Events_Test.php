...
namespace effcore\modules\test {
  ...
  static function set_roles(&$test, &$c_scenario, &$c_step, &$c_results, $nickname, $roles = [], $is_reset = false) {
    ...
    if ($user) {
      ...
      if ($is_reset) $reports[] = new text('there will be try to delete all roles');
      if (true     ) $reports[] = new text('there will be try to insert roles: %%_roles', ['roles' => implode(', ', $roles) ?: 'n/a']);
      $old_id_roles = user::relation_role_select    ($user->id);
      if ($is_reset)  user::relation_role_delete_all($user->id);
