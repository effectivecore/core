...
namespace effcore {
  ...
  # make request
    ...
    if (isset($response['headers']['X-PHP-Memory-usage'        ])) $c_results['reports'][$dpath][] = new text('&ndash; response header param "%%_name" = "%%_value"', ['name' => 'X-PHP-Memory-usage',         'value' => locale::format_bytes  ($response['headers']['X-PHP-Memory-usage'])        ]);
    if (isset($response['headers']['X-Time-total'              ])) $c_results['reports'][$dpath][] = new text('&ndash; response header param "%%_name" = "%%_value"', ['name' => 'X-Time-total',               'value' => locale::format_msecond($response['headers']['X-Time-total'])              ]);
