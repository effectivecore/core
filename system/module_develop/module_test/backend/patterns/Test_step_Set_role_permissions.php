...
class Test_step_Set_role_permissions {
    ...
    function run(&$test, $dpath, &$c_results) {
        ...
        if (true           ) $c_results['reports'][$dpath][] = new Text('there will be try to append permissions: %%_permissions', ['permissions' => implode(', ', $this->permissions) ?: 'n/a']);
        ...
        $c_results['reports'][$dpath][] = new Text('permissions before appending: %%_permissions', ['permissions' => implode(', ', $old_permissions) ?: 'n/a']);
        $c_results['reports'][$dpath][] = new Text('permissions after appending: %%_permissions' , ['permissions' => implode(', ', $new_permissions) ?: 'n/a']);
    }
