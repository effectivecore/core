...
class Test_step_Set_user_roles {
    ...
    function run(&$test, $dpath, &$c_results) {
        ...
        if ($user) {
            ...
            if (true           ) $c_results['reports'][$dpath][] = new Text('there will be try to append roles: %%_roles', ['roles' => implode(', ', $this->roles) ?: 'n/a']);
            ...
            $c_results['reports'][$dpath][] = new Text('roles before appending: %%_roles', ['roles' => implode(', ', $old_roles) ?: 'n/a']);
            $c_results['reports'][$dpath][] = new Text('roles after appending: %%_roles' , ['roles' => implode(', ', $new_roles) ?: 'n/a']);
        }
    }
