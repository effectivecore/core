...
namespace effcore {
  ...
  function run(&$test, $dpath, &$c_results) {
    if ($this->check === 'token') {
      ...
      if ($result === true) $c_results['reports'][$dpath.'/on_success_title'] = new text_multiline(['checking on "%%_operand_1" %%_operator "%%_operand_2"', 'real checking on "%%_operand_real_1" %%_operator "%%_operand_real_2"', 'result of checking is = "%%_result"'], ['operand_1' => $this->where, 'operand_2' => $this->match, 'operand_real_1' => $operand_1, 'operand_real_2' => $operand_2, 'operator' => $is_regexp ? (new text('matches'))->render() : '=', 'result' => (new text('success'))->render() ]);
