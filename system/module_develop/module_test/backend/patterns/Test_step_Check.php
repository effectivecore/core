...
namespace effcore {
  ...
  function run(&$test, $dpath, &$c_results) {
    if ($this->where === 'http_code') $result = array_key_exists('http_code',                  $c_results['response']['info'   ]) && $this->match === (int)$c_results['response']['info'   ]['http_code'                 ];
    if ($this->where === 'subm_errs') $result = array_key_exists('X-Form-Submit-Errors-Count', $c_results['response']['headers']) && $this->match === (int)$c_results['response']['headers']['X-Form-Submit-Errors-Count'];
