...
namespace effcore\modules\test {
  ...
  static function on_apply($name, $args = []) {
    ...
    if ($name === 'test_email_random'    && count($args) === 0) {                                                                                                                                            return 'test_'.core::hash_get_mini(random_int(0, PHP_INT_MAX_32)).'@example.com';}
    if ($name === 'test_nickname_random' && count($args) === 0) {                                                                                                                                            return 'test_'.core::hash_get_mini(random_int(0, PHP_INT_MAX_32));               }
    if ($name === 'test_password_random' && count($args) === 0) {                                                                                                                                            return         user::password_generate();                                        }
    if ($name === 'test_email_random'    && count($args) === 1) {if (!isset( static::$cache['test_email_random'   ][core::hash_get($args[0])] )) static::$cache['test_email_random'   ][core::hash_get($args[0])] = 'test_'.core::hash_get_mini(random_int(0, PHP_INT_MAX_32)).'@example.com'; return static::$cache['test_email_random'   ][core::hash_get($args[0])];}
    if ($name === 'test_nickname_random' && count($args) === 1) {if (!isset( static::$cache['test_nickname_random'][core::hash_get($args[0])] )) static::$cache['test_nickname_random'][core::hash_get($args[0])] = 'test_'.core::hash_get_mini(random_int(0, PHP_INT_MAX_32));                return static::$cache['test_nickname_random'][core::hash_get($args[0])];}
