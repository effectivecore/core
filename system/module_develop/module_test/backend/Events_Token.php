...
namespace effcore\modules\test {
  ...
  static function on_apply($name, $args = []) {
    ...
    if ($name === 'test_email_random'    && count($args) === 1) {if (!isset( static::$cache['test_email_random'   ][md5($args[0])] )) static::$cache['test_email_random'   ][md5($args[0])] = 'test_'.core::hash_get_mini(random_int(0, 0x7fffffff)).'@example.com'; return static::$cache['test_email_random'   ][md5($args[0])];}
    if ($name === 'test_nickname_random' && count($args) === 1) {if (!isset( static::$cache['test_nickname_random'][md5($args[0])] )) static::$cache['test_nickname_random'][md5($args[0])] = 'test_'.core::hash_get_mini(random_int(0, 0x7fffffff));                return static::$cache['test_nickname_random'][md5($args[0])];}
    if ($name === 'test_password_random' && count($args) === 1) {if (!isset( static::$cache['test_password_random'][md5($args[0])] )) static::$cache['test_password_random'][md5($args[0])] =         core::password_generate();                                     return static::$cache['test_password_random'][md5($args[0])];}
