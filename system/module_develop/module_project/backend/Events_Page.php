...
namespace effcore\modules\project {
  ...
  static function on_block_presets_dynamic_build($event, $id = null) {
    if ($id === null                                                  ) {foreach (entity::get('project')->instances_select() as $c_item) block_preset::insert('block__release_last_sql__'.$c_item->id, 'Release (last)', 'id_project = '.$c_item->id, [ /* all areas */ ], ['title' => 'Release (last)', 'title_is_visible' => false, 'type' => 'code', 'source' => '\\effcore\\modules\\project\\events_page::block_markup__selection_release_last_make', 'args' => ['id_project' => $c_item->id], 'has_admin_menu' => false, 'attributes' => ['data-block' => true, 'data-id' => 'block__release_last_sql__'.$c_item->id]], 0, 'project');}
    ...
    if ($id !== null && strpos($id, 'block__release_last_sql__') === 0) {$c_item__id = substr($id, strlen('block__release_last_sql__')); block_preset::insert('block__release_last_sql__'.$c_item__id, 'Release (last)', 'id_project = '.$c_item__id, [ /* all areas */ ], ['title' => 'Release (last)', 'title_is_visible' => false, 'type' => 'code', 'source' => '\\effcore\\modules\\project\\events_page::block_markup__selection_release_last_make', 'args' => ['id_project' => $c_item__id], 'has_admin_menu' => false, 'attributes' => ['data-block' => true, 'data-id' => 'block__release_last_sql__'.$c_item__id]], 0, 'project');}
  ...
  static function block_markup__selection_release_last_make($page, $args = []) {
    if (!empty($args['id_project'])) {
      token::insert('id_project_context', 'text', $args['id_project']);
      $selection = core::deep_clone(selection::get('release_last'));
