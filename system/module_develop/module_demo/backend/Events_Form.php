...
namespace effcore\modules\demo {
  ...
  static function on_submit_demo($form, $items, &$values) {
    ...
    if ($items['#email']->value_get() != 'test1@example.com,test2@example.com')                   message::insert(translation::get('Field "%%_title" has a changed value.', ['title' => translation::get($items['#email']->title)]));                                        # effcore\field_email
    ...
    if ($items['#select']->values_get() != ['option_1' => 'Option 1 (selected)'])                 message::insert(translation::get('Field "%%_title" has a changed value.', ['title' => translation::get($items['#select']->title)]));                                       # effcore\field_select
