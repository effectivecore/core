  ...
  static function on_submit($event, $form, $items) {
    ...
    switch ($form->clicked_button->value_get()) {
      case 'send':
        ...
        if ($items['#datetime_local' ]->value_get  ()       != '2030-02-01 01:23:45'     ) message::insert( new text('Field "%%_title" has a changed value.', ['title' => (new text($items['#datetime_local' ]->title))->render() ]) ); # …\field_datetime_local
        ...
        if ($items['*access'         ]->value_get_complex() != []                        ) message::insert( new text('Group "%%_title" has a changed value.', ['title' => (new text($items['*access'         ]->title))->render() ]) ); # …\widget_access
