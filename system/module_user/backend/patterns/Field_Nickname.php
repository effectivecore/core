...
namespace effcore {
  ...
  function render_description() {
    if (!$this->description)
         $this->description = new text('Field can contain only the next characters: %%_characters', ['characters' => static::allowed_characters_title]);
  ...
  static function validate_value($field, $form, $element, &$new_value) {
    if (strlen($new_value) && !core::validate_nickname($new_value)) {
      $field->error_set(
        'Field "%%_title" contains incorrect value!', ['title' => translation::apply($field->title)]
      );
    } else {
      return true;
    }
